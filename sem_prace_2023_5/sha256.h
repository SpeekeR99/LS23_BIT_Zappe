/**
 * Převzato a upraveno z: https://github.com/hlilje/sha-256
 */
...
std::string sha256(const std::string &str) {
    ...
    store_message_bytes(hex_string);
    ...
    std::stringstream ss;
    for (int i = 0; i < 8; ++i)
        ss << std::hex << std::setw(8) << std::setfill('0') << H[N][i];
...
std::vector<std::string> sha256(const std::vector<std::string> &strs) {
    std::vector<std::string> hashes;
    for (const std::string &str : strs)
        hashes.push_back(sha256(str));
    return hashes;
}
